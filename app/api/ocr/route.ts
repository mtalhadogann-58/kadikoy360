 ...
 try{
  ...
  const r=await fetch("https://api.openai.com/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({
   model:"gpt-4.1-mini",temperature:0,response_format:{type:"json_object"},messages:[
    {role:"system",content:"Türkiye milletvekili seçimi sandık sonuç tutanağından veri çıkar. Yalnızca fotoğrafta açıkça görülen değerleri yaz. Asla tahmin etme veya sayı uydurma. Okunmayan metin boş string, okunmayan sayı null olsun. Parti/siyasi parti oyları ile ittifakın ortak oylarını kesinlikle birbirine karıştırma. İttifak başlığının altındaki siyasi parti oylarını parties alanına, doğrudan ittifak hanesine verilmiş ortak oyları alliances alanına yaz. Aynı oyu iki alana birden yazma. Yalnız JSON döndür."},
    {role:"user",content:[{type:"text",text:'Şu JSON yapısıyla oku: {"school":"","ballotBox":"","district":"","neighborhood":"","registeredVoters":null,"voters":null,"validVotes":null,"invalidVotes":null,"parties":[{"name":"","votes":null}],"alliances":[{"name":"","votes":null}],"confidence":0,"warnings":[]}. Parti/aday satırlarını parties içine, yalnızca doğrudan ittifak hanesine basılan mühürlerden doğan ittifak ortak oylarını alliances içine görüldüğü sırayla ekle. İttifak adı görünmesine rağmen oy hanesi boşsa votes null yaz. confidence 0-100.'},{type:"image_url",image_url:{url:image,detail:"high"}}]}
   ]})});
